...
export async function POST(req: Request) {import Stripe from 'stripe'
import { NextResponse } from 'next/server'
import { createClient } from '@supabase/supabase-js'
...
export async function POST(req: Request) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { garage_id: garageId },
    success_url: `${APP_URL}/fr/success?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${APP_URL}/fr/cancel`,
  })
  ...
  // 1️⃣ Cherche si le garage existe déjà
  const { data: existing, error: selectError } = await supabase
    .from('garages')
    .select('id')
    .eq('email', email)
    .single() // récupère 1 ligne max

  let garageId: string

  if (selectError || !existing) {
    // garage inexistant → insert
    garageId = crypto.randomUUID()
    const { error: insertError } = await supabase
      .from('garages')
      .insert([{ id: garageId, email, payment_status: 'pending' }])
  ...
  // 2️⃣ Crée la session Stripe
  const session = await stripe.checkout.sessions.create({
    mode: 'subscription',
    customer_email: email,
    line_items: [{ price: process.env.STRIPE_PRICE_ID!, quantity: 1 }],
    subscription_data: { trial_period_days: 30, metadata: { garage_id: garageId } },
    metadata: { garage_id: garageId },
  ...
  // 3️⃣ Met à jour Supabase avec stripe_session_id
  const { error: updateError } = await supabase
    .from('garages')
    .update({ stripe_session_id: session.id })
    .eq('id', garageId)
  ...
  return NextResponse.json({ url: session.url, stripe_session_id: session.id, garage_id: garageId })
}
